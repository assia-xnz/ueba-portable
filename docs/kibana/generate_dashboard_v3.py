...
# ── 13. Top entités à risque (utilisateur × jour, classé) — amélioration précision ──
objs.append(
    lens(
        "ueba-v3-entity-table",
        "Top entités à risque (utilisateur × jour)",
        "lnsDatatable",
        "l13",
        {
            "user": c_terms("user", 20, "strong", "Utilisateur"),
            "day": c_terms("day", 5, "strong", "Jour"),
            "strong": c_metric("max", "strong_count", "Votes forts (≥3)"),
            "cnt": c_metric("max", "anomaly_count", "Fenêtres anormales"),
            "risk": c_metric("max", "max_risk_score", "Risk score", fmt=fmt1),
        },
        ["user", "day", "strong", "cnt", "risk"],
        {
            "layerId": "l13",
            "layerType": "data",
            "columns": [
            ...
            "sorting": {"columnId": "strong", "direction": "desc"},
            "rowHeight": "single",
            "rowHeightLines": 1,
        },
        DV_ENTITY,
        description="Entités classées par risque : les attaques (votes forts) remontent en tête.",
    )
)
...
panels = [
    ...
    ("ueba-v3-entity-table", 0, 62, 48, 16),
]
panels_json, refs = [], []
